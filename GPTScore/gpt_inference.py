...
# class GPT3Model(object):
#
# 	def __init__(self, rev_chatgpt_config: dict, logger=None):
# 		self.rev_chatgpt_config = rev_chatgpt_config
# 		self.rev_chatgpt = MyChatbot(config=rev_chatgpt_config)
# 		self.logger = logger
# 		self.tokenizer = GPT2Tokenizer.from_pretrained("gpt2-xl")
#
# 	def do_inference(self, input, output, max_length=2048):
...
# 		while not received:
# 			try:
# 				# response = self.rev_chatgpt.get_response(prompt)
# 				received = True
# 			except:
# 				error = sys.exc_info()[0]
# 				print("API error:", error)
